...
while true
	...
	#do curl -s 'https://youtube.googleapis.com/youtube/v3/liveChat/messages?liveChatId=KicKGFVDUjBvTm5ObU5JX0x6Nk0ckVHUkU&part=snippet&key=AIzaSaBGjk5tdwPI' --header "Authorization: Bearer $access_token" --header 'Accept: application/json' --compressed > json.txt
	...
	if [[ $last_message_id < $total_msg ]]; then
		if [[ $last_message_id == 0 ]]; then
			bash -c "echo -n 0:" ; channel_id=$(jq -r ".items[0].snippet.authorChannelId" json.txt) ; channel_url=$(curl -s https://www.googleapis.com/youtube/v3/channels\?id\=$channel_id\&part\=snippet\&key\=AIzaSyccBGk5tdwPI | jq -r ".items[].snippet.customUrl" | cut -c 2- ) ; zsh -c "echo -n $channel_url:" ; jq -r ".items[0].snippet.textMessageDetails.messageText" json.txt ; last_message_id=1 #if [[ $total_msg > 0 ]]; then last_message_id=1 ;fi
		else
			for ((i = last_message_id=1; i <= total_msg ; i=i+1)); do bash -c "echo -n $i:" ; channel_id=$(jq -r ".items[$i].snippet.authorChannelId" json.txt) ; channel_url=$(curl -s https://www.googleapis.com/youtube/v3/channels\?id\=$channel_id\&part\=snippet\&key\=AIzaSxaBjk5tdwPI | jq -r ".items[].snippet.customUrl" | cut -c 2- ) ; zsh -c "echo -n $channel_url:" ; jq -r ".items[$i].snippet.textMessageDetails.messageText" json.txt; done
